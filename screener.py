...
def run_screening(progress_callback=None):
    ...
    for i, ticker in enumerate(tickers):
        ...
        try:
            ...
            # QQE 대신 RSI 기울기로 모멘텀 판단
            rsi_now  = float(df['RSI'].iloc[-1])
            rsi_prev = float(df['RSI'].iloc[-4])
            cond_qqe = rsi_now > rsi_prev and rsi_now > 50

            r  = df.iloc[-10:]; p = df.iloc[-40:-10]
            ri = r['Close'].idxmin(); pi = p['Close'].idxmin()
            cond_div = bool(
                df.loc[ri,'Close'] < df.loc[pi,'Close'] and
                df.loc[ri,'RSI']   > df.loc[pi,'RSI']
            )
            ...
            if score >= 2:
                results.append({
                    ...
                    '>SMA200': cond_sma, 'RSI 모멘텀': cond_qqe,
                    '수급(1.5x)': cond_vol, 'RSI 다이버전스': cond_div,
